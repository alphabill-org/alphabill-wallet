...
type (
	...
	LockedUnit struct {
		...
		SystemID     types.SystemID `json:"systemId"`     // target system id of the locked unit
...
func NewLockedUnit(accountID, unitID, txHash []byte, systemID types.SystemID, lockReason LockReason, transactions ...*Transaction) *LockedUnit {
	return &LockedUnit{
		AccountID:    accountID,
		UnitID:       unitID,
		TxHash:       txHash,
...
func (l *LockedUnit) isValid() error {
	...
	if l.SystemID == 0 {
		return errors.New("system id is unassigned")
	}
	if l.TxHash == nil {
